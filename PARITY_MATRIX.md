...
| reco-calibrate | AKAZE/audio/lens DB | `cpp/reco_calibrate` | In progress for config/data types, deterministic geometry/reprojection helpers, descriptor matching, spatial overlap filtering, frame sampling/downscale helpers, RANSAC fundamental-matrix inlier selection, default matcher/filter/cost policies, Nelder-Mead optimizer, and lens-profile JSON lookup/search | Deferred: AKAZE detector, audio/video sampling, telemetry, embedded lens DB resource packaging; add fixtures, resource packaging, tolerance tests |
