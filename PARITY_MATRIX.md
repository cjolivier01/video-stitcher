...
| reco-core data/math | `crates/reco-core` non-GPU types | `cpp/reco_core` | In progress for `ViewportPosition`, rig correction, source path validation, frame data, calibration validation/JSON | Projection/lens tests, full schema goldens, fuzz parity runtime |
