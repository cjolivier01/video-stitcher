...
| reco-core data/math | `crates/reco-core` non-GPU types | `cpp/reco_core` | In progress for `ViewportPosition`, rig correction, source path validation, frame data, calibration validation/JSON, point-in-polygon, virtual-camera yaw/pitch, telemetry aggregation, frame timing micros, event frame-index vocabulary, replay buffer policy | Deferred: coverage boundary, camera/panorama projection chains, lens undistort, replay management/recorder traits; add full projection/lens tests and fuzz parity runtime |
