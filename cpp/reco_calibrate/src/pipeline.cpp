...
BackendProbe probe_from_runtime(const reco::io::RuntimeProbe& probe) {
  return {.available = probe.available,
          .detail = probe.available ? probe.library : probe.error};
}
...
validate_gpu_calibration_request(const GpuCalibrationRequest& request) {
  ...
  if (request.config.skip_start_secs < 0.0 || request.config.skip_end_secs < 0.0) {
    return "calibration skip durations must be non-negative";
  }
  if (request.config.matching.lowe_ratio <= 0.0 || request.config.matching.lowe_ratio > 1.0) {
    return "calibration Lowe ratio must be in (0, 1]";
  }
  if (request.config.akaze.threshold <= 0.0) {
    return "calibration AKAZE threshold must be positive";
  }
  if (request.config.akaze.detect_y_min < 0.0 || request.config.akaze.detect_y_max > 1.0 ||
...
CalibrationBackendStatus probe_calibration_backends() {
  ...
  status.cuda.detail =
      status.cuda.available ? "CUDA driver/runtime available" : reco::core::CudaBackend::availability_error();
...
CalibrationExecutionPlan build_gpu_calibration_plan(const GpuCalibrationRequest& request,
  ...
  append_step(plan, CalibrationStep::DetectingProfiles,
              request.left.lens_profile.has_value()
                  ? "load explicit left/right lens profiles"
